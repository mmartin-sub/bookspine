...
This module tests that files in the resources folder can be processed
correctly by the KTE module.
"""
...
RESOURCES_DIR = Path(__file__).parent.parent.parent.parent.parent / "tests" / "resources"
...
class TestKTEResources:
    ...
    def test_supported_files_exist(self):
        ...
        matching_files = [f for f in files if f.suffix in supported_suffixes]
        assert len(matching_files) > 0, f"No supported files found. Available files: {[f.name for f in files]}"
