"""
Unit tests for BookMetadata model.

This module tests the BookMetadata model which represents book specifications
for spine width calculation.
"""
...
class TestBookMetadata(unittest.TestCase):
    ...
    def test_invalid_page_count(self):
        ...
        with self.assertRaises(ValidationError) as context:
            BookMetadata(page_count=0)  # Use 0 instead of string
        self.assertIn("Page count must be positive", str(context.exception))
    ...
    def test_invalid_paper_weight(self):
        ...
        with self.assertRaises(ValidationError) as context:
            BookMetadata(page_count=200, paper_weight=-10)  # Use negative number instead of string
        self.assertIn("Paper weight must be positive", str(context.exception))
